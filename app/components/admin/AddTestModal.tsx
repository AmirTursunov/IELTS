...
interface Passage {
  ...
  title: string;
  content: string;
  hasParagraphs?: boolean; // With Paragraph turi
  paragraphs?: string[]; // Paragraph textlari
  hasInputParagraphs?: boolean; // With Input Paragraphs (drag & drop uchun)
...
export const AddTestModal: FC<AddTestModalProps> = ({ onClose, onSuccess }) => {
  const [formData, setFormData] = useState<ReadingTestForm>({
    title: "",
    difficulty: "Medium",
    timeLimit: 60,
    testType: "Academic",
    status: "paid",
    passages: [
      { passageNumber: 1, title: "", content: "", questionGroups: [] },
      { passageNumber: 2, title: "", content: "", questionGroups: [] },
      { passageNumber: 3, title: "", content: "", questionGroups: [] },
    ],
  });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl max-w-6xl w-full shadow-2xl h-[90vh] flex flex-col">
        ...
        <div className="p-6 border-b flex justify-between bg-gray-50 rounded-t-xl">
          <div>
            <h2 className="text-2xl font-bold">Create Reading Test</h2>
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={() => {
                            ...
                            ps[pIdx].hasInputParagraphs = false;
                            ps[pIdx].paragraphs = ["", "", ""]; // Default 3 paragraphs
                            setFormData({ ...formData, passages: ps });
                          }}
                          className="text-blue-600"
                        />
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={() => {
                            ...
                            ps[pIdx].paragraphs = ["", "", ""]; // Default 3 paragraphs with inputs
                            setFormData({ ...formData, passages: ps });
                          }}
                          className="text-blue-600"
                        />
